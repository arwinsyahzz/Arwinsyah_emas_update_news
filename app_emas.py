...
df_history = df_history.dropna(subset=['waktu'])
df_history = df_history.sort_values('waktu')

KURS_USD = 15700  # contoh
OZ_TO_GRAM = 31.1035

KURS_USD = 14500        # bisa kamu update
OZ_TO_GRAM = 31.1035   # konstanta resmi

df_history['harga'] = (
    df_history['harga'] * KURS_USD / OZ_TO_GRAM
)
df_history = df_history[df_history['harga'] > 100_000]

if df_history.empty:
    st.warning("Database belum berisi data harga emas")
else:
    st.metric(
        "Harga Emas Dunia (1 Gram)",
        f"Rp{df_history['harga'].iloc[-1]:,.0f}"
    )
# =========================
# CHART HARGA EMAS
# =========================
if not df_history.empty:
    ...
    harga_awal = df_history['harga'].iloc[0]
    harga_akhir = df_history['harga'].iloc[-1]

    selisih = harga_akhir - harga_awal
    persen = (selisih / harga_awal) * 100
    naik = selisih >= 0

    warna_garis = "green" if naik else "red"
    warna_area = "rgba(0,255,0,0.25)" if naik else "rgba(255,0,0,0.25)"

    st.markdown(
        f"""
        ### {"🟢 NAIK ⬆️" if naik else "🔴 TURUN ⬇️"}
        **Rp{abs(selisih):,.0f} ({persen:.2f}%)**
        """,
        unsafe_allow_html=True
    )
    ...
    fig.add_trace(go.Scatter(
        ...
        y=df_history['harga'],
        mode="lines",
        line=dict(color=warna_garis, width=3),
        fill="tozeroy",
        fillcolor=warna_area,
        hovertemplate="Waktu: %{x}<br>Harga: Rp%{y:,.0f}<extra></extra>"
    ))
    ...
    st.warning("Data harga emas belum tersedia")
